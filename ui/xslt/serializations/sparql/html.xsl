...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:res="http://www.w3.org/2005/sparql-results#"
	...
	<xsl:template match="/">
		<html lang="en">
			<head>
				<title><xsl:value-of select="/content/config/title"/>: SPARQL Results</title>
			...
			<xsl:choose>
				<xsl:when test="res:uri">
					...
					<xsl:variable name="link">						
						<xsl:choose>
							<xsl:when test="contains($uri, 'collection.britishmuseum.org')">
								...
								<xsl:value-of select="concat($display_path, 'query?query=', encode-for-uri($describe), '&amp;output=html')"/>
							</xsl:when>
							<xsl:otherwise>
